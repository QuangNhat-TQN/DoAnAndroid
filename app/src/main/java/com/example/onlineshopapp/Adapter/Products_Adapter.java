...
public class Products_Adapter extends RecyclerView.Adapter<Products_Adapter.MyViewHolder>{
    ...
    public Products_Adapter(List<Product> pro_List, Context context) {
    ...
    public void onBindViewHolder(@NonNull Products_Adapter.MyViewHolder holder, int position) {
        Product product = pro_List.get(position);
        holder.pro_name.setText(product.getName());
        holder.pro_price.setText(convertToVND(String.format("%dđ", product.getPrice())));
        holder.pro_rating.setText(String.format("%.1f", product.getRating()));
